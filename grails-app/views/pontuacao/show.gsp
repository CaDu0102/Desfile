	...
	<body>
		...
		<div id="show-pontuacao" class="content scaffold-show" role="main">
			...
			<ol class="property-list pontuacao">
				...
				<g:if test="${pontuacaoInstance?.carnavais}">
				<li class="fieldcontain">
					<span id="carnavais-label" class="property-label"><g:message code="pontuacao.carnavais.label" default="Carnavais" /></span>
					
						<g:each in="${pontuacaoInstance.carnavais}" var="c">
						<span class="property-value" aria-labelledby="carnavais-label"><g:link controller="carnaval" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
