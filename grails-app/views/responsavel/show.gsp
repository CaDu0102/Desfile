	...
	<body>
		...
		<div id="show-responsavel" class="content scaffold-show" role="main">
			...
			<ol class="property-list responsavel">
				...
				<g:if test="${responsavelInstance?.escolas}">
				<li class="fieldcontain">
					<span id="escolas-label" class="property-label"><g:message code="responsavel.escolas.label" default="Escolas" /></span>
					
						<g:each in="${responsavelInstance.escolas}" var="e">
						<span class="property-value" aria-labelledby="escolas-label"><g:link controller="escola" action="show" id="${e.id}">${e?.encodeAsHTML()}</g:link></span>
						</g:each>
					
				</li>
				</g:if>
			
				<g:if test="${responsavelInstance?.idEscola}">
				<li class="fieldcontain">
					<span id="idEscola-label" class="property-label"><g:message code="responsavel.idEscola.label" default="Id Escola" /></span>
					
						<span class="property-value" aria-labelledby="idEscola-label"><g:link controller="escola" action="show" id="${responsavelInstance?.idEscola?.id}">${responsavelInstance?.idEscola?.encodeAsHTML()}</g:link></span>
